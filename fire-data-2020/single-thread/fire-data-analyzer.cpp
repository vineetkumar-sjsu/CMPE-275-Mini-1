...
public:
    ...
    void loadCSVFile(const std::string& filename) {
        ...
        while (std::getline(file, line)) {
            ...
            try {
                // 2nd potential candidate for OMP parallelization
                ...
                std::cerr << "Error parsing line " << lineCount << " in " << filename 
                         << ": " << e.what() << std::endl;
            }
        }
    ...
    std::vector<AirQualityRecord> getAQIDataForDate(const std::string& targetDate) {
        ...
        for (const auto& record : records) {
            ...
            // make sure other threads stop working if record are found
